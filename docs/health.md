...
Liveness reports only that the HTTP process can respond. Startup is unavailable
in `new` and `starting` and remains successful after startup completes.
...
the shared semaphore caps active checks across concurrent requests, and
`MaxChecks` caps per-request scheduling. Hard construction limits are 256
concurrent checks and 1024 registered checks.
