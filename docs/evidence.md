...
| business and management HTTP use correlation-owned ingress identity | `serverhttp.WithCorrelation` and `correlation/adapters/http` | `TestServeCommandOwnsBusinessHTTPWithCorrelationAndTraceOrder`, `TestLongRunningCommandServesCanonicalManagementProbes` | `docs/http.md`, `docs/platform/contracts.md` |
| logs and provider-neutral events carry bounded process and request identity | enriched `BuildContext.Logger`, `RuntimeObserver`, and probe/request observation | `TestCohesivePlatformEnrichesLogsAndReportsRuntimeBoundaries`, `TestManagementProbesReportResultsWithoutLoggingSuccessfulRequests`, component failure and panic regressions | `docs/observability.md`, `docs/platform/contracts.md` |
| optional maintenance coordinates commands, admission, bypass, and readiness | `MaintenanceStore`, built-in commands, admission middleware, and readiness overlay | `TestCohesiveMaintenanceCommandsControlAdmissionReadinessAndBypass`, `TestMaintenanceCustomResponseControlsHeadersWhilePlatformForces503`, store and refresh regressions | `docs/maintenance.md`, `docs/health.md` |
...
| correlation-owned HTTP identity | `WithCorrelation`, `correlation/adapters/http` | `TestCorrelationAndIngressRunBeforeBodyRejection`, `TestRecoveryPreservesSafeIdentityAndSecurityHeaders` | `docs/http.md`, `docs/security.md` |
...
| every probe result is observable without per-success transition logging | `healthhttp.Observer` and cohesive probe state tracking | `TestProbeObserverReceivesResultsAndCannotBreakResponses`, `TestManagementProbesReportResultsWithoutLoggingSuccessfulRequests` | `docs/health.md`, `docs/observability.md` |
...
| Promise | Implementation | Executable evidence | Public contract |
| --- | --- | --- | --- |
| every mandatory owning module adapts its concrete resource without reversing the core dependency graph | canonical Config, PostgreSQL, Kafka, Lease, Queue, Telemetry, and Migrations service adapters plus the retained Cache and Scheduler compatibility paths | each adapter package's tests and examples; `TestOwningModuleAdaptersComposeIntoReferenceDefinitions` | `docs/platform/dependencies.md`, owning-module service-integration guides |
