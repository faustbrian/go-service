...
| configuration failure prevents partial startup without leaking sensitive source text | `integration.New` as first component and `go-config` sensitive-source errors | `TestHookComponentPreventsPartialStartupAndKeepsCleanupOrdered`, `TestActualConfigurationFailureIsRedactedAndPreventsStartup`, `ExampleNew` | `docs/configuration.md`, `docs/integration.md` |
