...
`go-service` owns process-local runtime composition. It does not load
...
may use `go-config`, plain flags, environment variables, or any other source.
`go-service` accepts constructed configuration values but contains no source,
