...
Supervised goroutines receive the service context. Returning an error or
panicking cancels the service with a typed cause. Shutdown joins all supervised
work within the caller's bound. A goroutine that ignores cancellation can make
shutdown return the caller's context error, but it cannot be abandoned or
reported as joined.
