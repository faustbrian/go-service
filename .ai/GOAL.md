...
- startup configuration integration through `go-config`
- integration hooks for standard `*slog.Logger` values and `go-log`
- integration hooks for OpenTelemetry providers and `go-telemetry`
- composition examples and optional adapters for `go-authentication`,
  `go-authorization`, `go-scheduler`, and `go-queue`
...
- `integration` for optional `go-log` and `go-telemetry` runtime wiring
...
- middleware composition cookbook including `go-authentication` and
  `go-authorization`
- observability integration guide
- `go-config` startup and secret-delivery integration guide
