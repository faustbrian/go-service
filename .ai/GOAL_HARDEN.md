...
- relevant HTTP semantics and Kubernetes probe/lifecycle
  documentation
- `go-log`, `go-telemetry`, and OpenTelemetry contracts used by optional
...
- Verify `go-config` load failures prevent partial service startup and preserve
  secret-safe errors, cancellation, and cleanup.
- Verify `go-authentication`, `go-authorization`, `go-scheduler`, and `go-queue`
  integrations preserve middleware order, cancellation, and dependency direction.
- Prove `go-service` does not implement configuration loading, authentication,
...
- Audit log attributes, trace propagation, metric labels, disabled telemetry,
  and duplicate registration through `go-log` and `go-telemetry` integrations.
- Prove `go-service` does not initialize exporters, own logging handlers, or
